...
<c:choose>
	<c:when test="${not empty manuscript.files }">
		...
		<c:forEach var="f" items="${manuscript.files}">
			<c:if test="${f.revisionCount == manuscript.revisionCount and (f.designation == 'frontCover' or f.designation == 'checkList')}">
						...
						<c:forEach var="f" items="${manuscript.files}">
							<c:if test="${f.revisionCount == manuscript.revisionCount and (f.designation == 'frontCover' or f.designation == 'checkList')}">
								<tr>
									...
									<td class="cellCenter">
										<c:choose>
											<c:when test="${f.designation == 'frontCover'}">
												<spring:message code="author.newPaperSubmit.additionalFileDesignation.0"/> (<spring:message code="system.required"/>)
											</c:when>
											<c:when test="${f.designation == 'checkList'}">
												<spring:message code="author.newPaperSubmit.additionalFileDesignation.1"/> (<spring:message code="system.required"/>)
											</c:when>
										</c:choose>
									</td>
									<td class="cellCenter">
