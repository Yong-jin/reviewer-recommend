<%@ tag body-content="empty" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ attribute name="comment" type="link.thinkonweb.domain.manuscript.form.Comment"%>
<%@ attribute name="locale" %>
<%@ attribute name="currentRole" %>
<c:if test="${currentRole == 'chiefEditor' }">
<img class="avatar img-responsive" alt="Chief Editor" src="images/icons/avatar/ce.png"/>
</c:if>
<c:if test="${currentRole == 'guestEditor' }">
<img class="avatar img-responsive" alt="Guest Editor" src="images/icons/avatar/ge.png"/>
</c:if>
<div class="message">
	<span class="arrow">
	</span>
	<span class="name">
		<c:choose>
			<c:when test="${journal.languageCode ==  'ko' and fn:trim(comment.fromUser.contact.localFullName) != ''}">, 
				${comment.fromUser.contact.localFullName }
			</c:when>
			<c:otherwise>
				<c:if test="${comment.fromUser.contact.salutation != '-1'}"><spring:message code="signin.salutationDesignation.${comment.fromUser.contact.salutation}"/></c:if> ${comment.fromUser.contact.firstName } ${comment.fromUser.contact.lastName }, 
			</c:otherwise>
		</c:choose>
		<c:if test="${currentRole == 'chiefEditor' }">
			<spring:message code="user.role.journal_c-editor"/>
		</c:if>
		<c:if test="${currentRole == 'guestEditor' }">
			<spring:message code="user.role.journal_g-editor"/>
		</c:if>
	</span>
	<span class="chatSubject">
		<c:if test="${currentRole == 'chiefEditor' }">
			<spring:message code="messages.forceToRejectByChiefEditor"/>
		</c:if>
		<c:if test="${currentRole == 'guestEditor' }">
			<spring:message code="messages.forceToRejectByGuestEditor"/>
		</c:if>
	</span>
	<span class="body">
		${comment.textHtml}
		<small id="forceToRejectDate${comment.revisionCount }"></small>
		<script type="text/javascript">
			document.getElementById("forceToRejectDate${comment.revisionCount }").innerHTML = "(" + convertUTCDateToLocalDate("${comment.date}", "${comment.time}", "${locale}") + ")";
		</script>
	</span>
</div>