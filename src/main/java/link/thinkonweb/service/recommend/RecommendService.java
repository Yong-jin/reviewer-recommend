...
public class RecommendService {
	...
	private double u_weight_value;
	private int recommend_extra_ratio;
	...
	public void setFR_Period_days(int FR_period_days) {  //빈도수 계산 제한 날짜
		this.FR_period_days = FR_period_days;
	}

	public void setU_Weight_Value(double wv) {  // U함수와 V함수에 대한 가중치
		this.u_weight_value = wv;
	}

	public void set_Recommend_Extra_Ratio(int er) { // 몇배수로 추천수
		this.recommend_extra_ratio = er;
	}
	...
	public double functionV(Manuscript m, SystemUser r) { //SystemUser 을 Reviewer의 개념으로 사용  //일단완료
		double result = 0;
		double sumU = 0;
		double sd = 0;
		keywords = m.getKeywords();
	...
	public double function_F(Manuscript m, SystemUser r) {  //functionF =  (a)FunctionU+(1-a)functionV   //일단완료 
		return u_weight_value*functionU(m, r) + (1-u_weight_value)*functionV(m,r);
	}
	...
	public double functionFR(SystemUser r) { //Reviewer가 온다고 생각 SystemUser을 사용하지만 //일단완료
		...
		review_num = reviewEventDateTimeDao.numReviewsBeforeSpecificDays(r.getId(), FR_period_days);
	...
	public boolean isReviewing(Manuscript m, Reviewer r) {  //둘사이에 리뷰 중인가. 일단완료
	...
	public HashMap<Manuscript, List<Reviewer>> recommend_Assignment()  //Main !!! //일단완료
	{
		...
		for(Manuscript m: manuscripts) {
			...
			for(int i=0; i<constraint_review_num_for_paper*recommend_extra_ratio; i++) { // f함수를 가지고 1차를 추천 목록 추려냄
				for(Reviewer r_init: reviewers) {   //가능한 리뷰어중 초기값
					if(isReviewing(m, r_init) == false && reviewers_1.contains(r_init) == false) {
						max_r = r_init;
						break;
					}
				}
				...
				for(Reviewer r: reviewers) {
					if(isReviewing(m, r) == false && reviewers_1.contains(r) == false) {  //현재 리뷰상태가 아닌  and 1차목록에 없는
						if( function_F(m, r.getUser()) > function_F(m, max_r.getUser()) )
								max_r = r;
					}
				}
			...
			for(int j=0; j<constraint_review_num_for_paper*recommend_extra_ratio; j++) {
	...
	public List<Reviewer> getPossibleReviewers() {  //일단 완료
		...
		for(Reviewer reviewer: reviewers) {
			...
			if(numCurrentReview < constraint_review_num_for_reviewer)
	...
	public List<Manuscript> getNeedManuscripts() {  //일단완료
		...
		for(Manuscript manuscript: need_Manuscripts) {
			...
			if(reviews != null)  //현재 논문 리뷰중인 애들 찾기
				if(reviews.size() >= constraint_review_num_for_paper)
